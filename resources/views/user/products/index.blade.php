...
@section('user')
    <div class="page-content">
        ...
        <form method="GET" action="{{ route('user.products.index') }}" class="mb-4">
            ...
            @foreach ($products as $product)
                <div class="col-md-4">
                    <div class="card mb-4">
                        ...
                        @if ($product->video)
                            <video id="videoPreview{{ $product->id }}" class="card-img-top video-preview" muted
                                preload="auto">
                        ...
                        <div class="card-body">
                            ...
                            <p class="mb-2"><strong>Seller:</strong> {{ $product->seller->name }}</p>

                            <!-- Buttons for Details and Report -->
                            <div class="d-flex justify-content-between">
                                <button type="button" class="btn btn-outline-danger" data-bs-toggle="modal"
                                    data-bs-target="#reportModal{{ $product->seller->id }}">
                                    Report
                                </button>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body" style="max-height: 400px; overflow-y: auto;">
                                ...
                                <h5>{{ $product->name }}</h5>
                                <p class="mb-3">Rp. {{ number_format($product->price, 2) }}</p>
                                <textarea name="description" class="form-control" id="productDescription" placeholder="{{ $product->description }}"
                                    maxlength="500" readonly style="width: 100%; height: 150px;"></textarea>
                                <p class="mt-2"><strong>Seller:</strong> {{ $product->seller->name }}</p>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-primary"
                                    onclick="confirmCheckout({{ $product->id }}, '{{ $product->name }}', '{{ number_format($product->price, 2) }}')">Buy</button>
                ...
                <div class="modal fade" id="reportModal{{ $product->seller->id }}" tabindex="-1"
                    aria-labelledby="reportModalLabel{{ $product->seller->id }}" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="reportModalLabel{{ $product->seller->id }}">Report Seller</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-label="Close"></button>
                            </div>
